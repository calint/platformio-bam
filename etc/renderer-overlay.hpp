...
template <bool TileTransparent = false, bool RenderSprites = true,
          uint32_t TileWidth = 16, uint32_t TileHeight = 16>
static inline auto
render_scanline(uint8_t const (*tile_imgs_array)[TileWidth * TileHeight],
                uint16_t* render_buf_ptr, sprite_ix* collision_map_row_ptr,
    ...
    while (remaining_x) {
        ...
        uint8_t const* tile_img_ptr = &tile_imgs_array[*tile_map_ptr][0];
        if (flip_vert) {
        ...
        while (render_n_pixels--) {
            if (TileTransparent) {
                uint16_t const pixel = palette_tiles[*tile_img_ptr];
                if (pixel != 0) {
...
inline auto render(int const x, int const y) -> void {
    ...
    tile_img_ix const* tile_map_overlay_row_ptr = &tile_map[tile_y][0];
    ...
    while (remaining_y) {
        ...
        while (tile_line < render_n_tile_lines) {
            render_scanline<false, false, tile_width, tile_height>(
                tile_imgs, render_buf_ptr, collision_map_row_ptr, tile_x,
                tile_x_fract, tile_map_row_ptr, tile_map_flags_row_ptr,
                scanline_y, tile_line_times_tile_width,
                tile_line_times_tile_width_flipped);
            render_scanline<true, true, tile_width, tile_height>(
                tile_imgs, render_buf_ptr, collision_map_row_ptr, tile_x,
                tile_x_fract, tile_map_overlay_row_ptr, tile_map_flags_row_ptr,
                scanline_y, tile_line_times_tile_width,
                tile_line_times_tile_width_flipped);
            // render_scanline<false, true, tile_width, tile_height>(
            //     tile_imgs, render_buf_ptr, collision_map_row_ptr, tile_x,
            //     tile_x_fract, tile_map_row_ptr, tile_map_flags_row_ptr,
            //     scanline_y, tile_line_times_tile_width,
            //     tile_line_times_tile_width_flipped);
        ...
        tile_map_overlay_row_ptr += tile_map_width;
