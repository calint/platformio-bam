...
class dma_buffers final {
  public:
    ...
    static int constexpr buf_size_B =
        sizeof(uint16_t) * display_width * dma_n_scanlines;

    auto init() -> void {
        buf_1_ = static_cast<uint16_t*>(calloc(1, buf_size_B));
        buf_2_ = static_cast<uint16_t*>(calloc(1, buf_size_B));
        if (!buf_1_ || !buf_2_) {
...
// allocated in 'setup()'
static int constexpr collision_map_size_B =
...
static auto render(int const x, int const y) -> void {
    ...
    int constexpr dma_n_scanlines_trailing = display_height % dma_n_scanlines;
